...
架构四阶段：
  阶段1: 多尺度特征提取
    - VisualEncoder : Frozen CLIP ViT-L/14 → 取第 6,12,18,24 层 → 投影到 768
  ...
  阶段3: 动态属性推理 (DynamicAttributeModule)
    PARN (ResNet-50 冻结) → 11 个 attribute tokens → CrossAttn
...
class VisualEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.clip = CLIPViTL14(config["clip_pretrained"])
...
class WeakInteractionFusion(nn.Module):
    """
    单层弱交互式融合（对应一个 CLIP 层级和一个 BERT 层级）
...
class ImprovedIAAModel(nn.Module):
    ...
    config 需包含以下字段：
      ...
      dropout          : 0.1
      vision_freeze    : bool（是否冻结 CLIP，当前固定冻结）
      parn_pretrained  : str 或 ""（AMM-Net.pt 路径，下方重复，可忽略）
